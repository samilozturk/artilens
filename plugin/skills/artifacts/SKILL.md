...
1. Run `artilens artifacts list`.
2. Read `.claude/artifacts/index.md` for low-token discovery.
3. If the user wants details, run `artilens artifacts show <slug>`.
4. If the user wants a version comparison, run `artilens artifacts diff <slug> <vA> <vB> --as-artifact --out .claude/artilens/<slug>-diff.html`.
